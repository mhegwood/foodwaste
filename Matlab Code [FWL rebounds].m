...
ConsumptionIncrease = zeros(121,6,8);
...
load('SI Model Input Data [FWL Rebounds].mat')
...
for FTI=1:6 %Food Type Index (FTI) test with :1 just cereals, change to :6 to include all food types
    ...
    for SDGI=1:8 %SDG Index (SDGI) Region test with :1 for just one region, change to :8 for all regions
...
for f = 1:6
    for r = 1:8
        ...
        tr = tinv([0.025 0.975], length(ReboundPct(:,f,r))-1); 
        P_r_l(r,f) = mean(ReboundPct(:,f,r)) + tr(1)*std(ReboundPct(:,f,r))/sqrt(length(ReboundPct(:,f,r))); %Lower percentile
        P_r_u(r,f) =  mean(ReboundPct(:,f,r)) + tr(2)*std(ReboundPct(:,f,r))/sqrt(length(ReboundPct(:,f,r))); %Upper percentile
        
        tc = tinv([0.025 0.975], length(ConsumptionIncrease(:,f,r))-1); 
        P_c_l(r,f) =  mean(ConsumptionIncrease(:,f,r)) + tc(1)*std(ConsumptionIncrease(:,f,r))/sqrt(length(ConsumptionIncrease(:,f,r))); %Lower percentile
        P_c_u(r,f) = mean(ConsumptionIncrease(:,f,r)) + tc(2)*std(ConsumptionIncrease(:,f,r))/sqrt(length(ConsumptionIncrease(:,f,r))); %Upper percentile
